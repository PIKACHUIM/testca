...
export const MESSAGES = {
  zh: {
    ...
    'prod.mtls.hint': '网站加密',
    'prod.sign.hint': '文档签名',
    'prod.ssh2.hint': 'SSH认证',
    'prod.acme.hint': 'ACME自动化',
    'prod.ssl3.hint': 'SSL证书',
    'prod.iots.hint': 'IoT设备',
    'prod.nets.hint': '网络设备',
    'prod.card.hint': '智能卡',
    'prod.fido.hint': 'FIDO认证',
    ...
    'prod.ssh2.use': '用于 SSH2 服务器 / 客户端身份认证，生成的密钥对可直接用于 OpenSSH 登录认证。ZIP 包内含 OpenSSH 格式私钥与 SSH2 公钥文件，可直接部署到 ~/.ssh/ 目录。',
    'prod.acme.use': '用于 ACME（自动证书管理环境）协议测试，模拟 Let\'s Encrypt 等自动化证书签发流程。适用于 ACME 客户端开发调试、自动续期测试及 DevOps 流水线集成验证。',
    'prod.ssl3.use': '用于签发 SSL/TLS 服务器证书，适用于 HTTPS 网站、API 网关、反向代理等场景的测试部署。支持 SAN 多域名与通配符，仅限内网 / 测试环境使用。',
    'prod.iots.use': '用于物联网（IoT）设备身份认证与通信加密，适用于 MQTT / CoAP / LwM2M 等协议的设备证书签发测试，可用于设备入网认证与固件安全启动验证。',
    'prod.nets.use': '用于网络设备（路由器、交换机、防火墙等）的身份认证与管理通道加密，适用于 802.1X、IPSec VPN、SNMPv3 等网络安全协议的证书测试。',
    'prod.card.use': '用于智能卡（Smart Card）/ USB Token 的证书签发测试，适用于 PIV、CAC、PKCS#11 等智能卡标准的开发调试，可用于门禁、登录认证等场景。',
    'prod.fido.use': '用于 FIDO2 / WebAuthn 无密码认证的证书测试，适用于安全密钥（YubiKey 等）的注册与认证流程开发调试，支持 FIDO 元数据服务（MDS）集成测试。',
  ...
  en: {
    ...
    'prod.ssh2.hint': 'SSH auth',
    'prod.acme.hint': 'ACME',
    'prod.ssl3.hint': 'SSL/TLS',
    'prod.iots.hint': 'IoT device',
    'prod.nets.hint': 'Network',
    'prod.card.hint': 'Smart card',
    'prod.fido.hint': 'FIDO/WebAuthn',
    ...
    'prod.ssh2.use': 'SSH2 server / client authentication. The generated key pair can be used directly for OpenSSH login. The ZIP bundle includes an OpenSSH-format private key and SSH2 public key file, ready to deploy to ~/.ssh/ directory.',
    'prod.acme.use': 'For testing ACME (Automatic Certificate Management Environment) protocol workflows, simulating automated certificate issuance like Let\'s Encrypt. Suitable for ACME client development, auto-renewal testing and DevOps pipeline integration.',
    'prod.ssl3.use': 'Issues SSL/TLS server certificates for HTTPS websites, API gateways and reverse proxies in test environments. Supports SAN multi-domain and wildcards. For internal / test use only.',
    'prod.iots.use': 'IoT device identity authentication and communication encryption. Issues device certificates for MQTT / CoAP / LwM2M protocol testing, device onboarding and secure boot verification.',
    'prod.nets.use': 'Network device (routers, switches, firewalls) identity authentication and management channel encryption. For 802.1X, IPSec VPN, SNMPv3 and other network security protocol certificate testing.',
    'prod.card.use': 'Smart card / USB token certificate issuance testing. For PIV, CAC, PKCS#11 smart card standard development and debugging, applicable to access control and login authentication scenarios.',
    'prod.fido.use': 'FIDO2 / WebAuthn passwordless authentication certificate testing. For security key (YubiKey, etc.) registration and authentication flow development, supports FIDO Metadata Service (MDS) integration testing.',
